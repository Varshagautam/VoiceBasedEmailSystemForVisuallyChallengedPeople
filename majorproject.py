...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:@localhost/blindemail'
    ...
    if request.method =="POST":
        ...
        if id ==0:
            while True:
                ...
                speak("For confirmation say yes or for cancelling say no")
                speak1("speak")
                option = is_confirm()
                if option =="yes":
...
def send2():
    if request.method =="POST":
        if len(request.form["email"])!=0:
            ...
            #     smtp.login(EMAIL_SENDER, PASS)
            #     smtp.sendmail(EMAIL_SENDER,receiveremail,em.as_string())
...
def sent1():
        ...
        if len(value1)>0:
            ...
            return render_template("sent.html",receiveremail=receiveremail,subject=subject,message=message,num=num)
    ...
    if request.method=="POST":
        ...
        if len(value1)>0:
            ...
            for i in range(len(value1)):
                sno = int(str(value1[i])[11:-1])
                data2 =  Sentemail.query.get(sno)
                speak(f"mail number {data2.sno} ")
                speak(f"Date {data2.date} ")
                speak(f"send to {data2.email}")
                speak(f"subject {data2.subject} ")
                speak(f"message {data2.message} ")
                time.sleep(1)
                speak1("Do you want to read futher email if yes say yes or no then say no")
                speak1("speak")
                count+=1
                choice = is_confirm()
                if choice =="yes" and count!=len(value1):
                    continue
                elif choice =="no":
                    return redirect("mainpage")
